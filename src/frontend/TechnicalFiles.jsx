 ...
 useEffect(() => {
  const fetchFiles = async () => {
    try {
      const data = projectId
        ? await filesApi.getTechnicalFiles(projectId)
        : await filesApi.getTechnicalFiles();
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const savedFile = await filesApi.uploadTechnicalFile(
        projectId,
        uploadedFile,
        userId
      );
  ...
  const handleDelete = async (id) => {
    try {
      await filesApi.deleteFile('technical_files', id);
